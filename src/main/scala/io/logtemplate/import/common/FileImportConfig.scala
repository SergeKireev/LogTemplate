...
sealed trait RawImportConfig extends ImportConfig {
  protected def scoped(): Config
...
case class FileImportConfig(config: Config) extends RawImportConfig {
  protected def scoped() = config.getConfig("file")
  def filePath: String = scoped.getOrElse("path", "src/resources/file.log")
}

case class OpenTelemetryConfig(config: Config) extends RawImportConfig {
  protected def scoped() = config.getConfig("opentelemetry")
